...
import net.de1mos.microdiary.authservice.domain.CreateInternalUserCommand
...
class Runner(private var commandGateway: CommandGateway, private val queryGateway: QueryGateway) : CommandLineRunner {
    ...
    override fun run(vararg args: String?) {
        ...
        val result: CompletableFuture<UUID> = commandGateway.send(CreateInternalUserCommand(UUID.randomUUID(), "fedor2", "fedor2@gmail.com", "qwerty"))
        println("Finish sending command")
        println(result.get())

        println("Start sending command 2")
        val result2: CompletableFuture<UUID> = commandGateway.send(CreateInternalUserCommand(UUID.randomUUID(), "fedor3", "fedor3@gmail.com", "qwerty"))
